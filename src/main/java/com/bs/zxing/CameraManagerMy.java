...
public final class CameraManagerMy {

    private static final String TAG = CameraManagerMy.class.getSimpleName();

    // 扫描框大小修改
    private static final int MIN_FRAME_WIDTH = 666;
    private static final int MIN_FRAME_HEIGHT = 666;
    private static final int MAX_FRAME_WIDTH = 666;
    private static final int MAX_FRAME_HEIGHT = 666;

    private static CameraManagerMy cameraManager;
     ...
     * message.
     */
    private final PreviewCallbackMy previewCallback;
    /**
     * Autofocus callbacks arrive here, and are dispatched to the Handler which
     * requested them.
     */
    private final AutoFocusCallbackMy autoFocusCallback;

    /**
     * Initializes this static object with the DeviceManager of the calling Activity.
     *
     * @param context The Activity which wants to use the camera.
     */
    public static void init(Context context) {
        if (cameraManager == null) {
            cameraManager = new CameraManagerMy(context);
        }
    }

    /**
     * Gets the CameraManagerMy singleton instance.
     *
     * @return A reference to the CameraManagerMy singleton.
     */
    public static CameraManagerMy get() {
        return cameraManager;
    }

    private CameraManagerMy(Context context) {

        this.context = context;
        this.configManager = new CameraConfigurationManagerMy(context);
        ...
        previewCallback = new PreviewCallbackMy(configManager,
                useOneShotPreviewCallback);
        autoFocusCallback = new AutoFocusCallbackMy();
    }
    ...
    public void openDriver(SurfaceHolder holder) throws IOException {
        if (camera == null) {
            ...
            // {
            // FlashlightManagerMy.enableFlashlight();
            // }
            FlashlightManagerMy.enableFlashlight();
        }
    }
    ...
    public void closeDriver() {
        if (camera != null) {
            FlashlightManagerMy.disableFlashlight();
            camera.release();
            camera = null;
        }
    }
     ...
     * @return A PlanarYUVLuminanceSourceMy instance.
     */
    public PlanarYUVLuminanceSourceMy buildLuminanceSource(byte[] data,
        ...
        switch (previewFormat) {
            ...
            case PixelFormat.YCbCr_422_SP:
                return new PlanarYUVLuminanceSourceMy(data, width, height, rect.left,
            ...
            default:
                ...
                if ("yuv420p".equals(previewFormatString)) {
                    return new PlanarYUVLuminanceSourceMy(data, width, height,
