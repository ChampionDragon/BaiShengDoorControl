...
public final class MyViewfinderView extends View {
	...
	public MyViewfinderView(Context context, AttributeSet attrs) {
		...
		maskColor = resources.getColor(R.color.my_viewfinder_mask);// 扫描框旁边区域的颜色
		resultColor = resources.getColor(R.color.red);
		resultPointColor = resources.getColor(R.color.orange);// 扫描框闪光点的颜色
	...
	public void onDraw(Canvas canvas) {
		...
		Rect frame = CameraManagerMy.get().getFramingRect();
		if (frame == null) {
			return;
		}
		...
		if (resultBitmap != null) {
			...
			canvas.drawText(
					...
					(float) (frame.left * 1.4),
					(float) (frame.bottom + (float) TEXT_PADDING_TOP * density),
					paint);// 根据文字的长短更改系数1.4,frame.left横坐标修改
